...
2. Recompute approval digest from currently approved finding IDs, tier/scope, exclusions, audit path, and approval timestamp. STOP if digest differs, approval is absent/stale, or finding content changed after approval.
...
- Return files changed, line delta, behavior, evidence, counters, remaining unknowns, and recovery instruction. On failure, return failed-wave manifest required by SKILL.md.
