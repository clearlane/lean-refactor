...
The coordinator records each repair or verification outcome through `scripts/record-boundary-result.sh`. Its JSON ledger owns separate repair-attempt and verification-failure counters, each limited to `2`, separate repair and verification statuses, aggregate boundary status, and the latest evidence manifest/hash. Retryable or failed results increment the relevant counter; exhaustion becomes `blocked`. A boundary becomes `completed` only after both repair and verification complete. Ledger validation re-hashes the latest evidence manifests, and wave finalization rejects any `pending` boundary. Workers receive the state path but never mutate workflow state themselves.
